import { UdpClient, bufferToHex } from "./commandCenter/udpClient.js";
...
async function run() {
    ...
    udp.on("message", (message) => {
        console.log("UDP async/raw:", message.remote.address, message.remote.port, bufferToHex(message.data));
    });
    try {
        await udp.open();
        console.log("Z21 status request...");
        const response = await udp.sendAndReceive(buildZ21Packet(LAN_SYSTEMSTATE_GETDATA), (message) => isZ21Header(message.data, LAN_SYSTEMSTATE_DATACHANGED), 3000);
...
function buildZ21Packet(header, data) {
...
function isZ21Header(buffer, header) {
    if (buffer.length < 4)
        return false;
...
function parseSystemState(buffer) {
    if (!isZ21Header(buffer, LAN_SYSTEMSTATE_DATACHANGED)) {
        throw new Error(`Invalid Z21 system state response header: ${bufferToHex(buffer)}`);
    }
    const packetLength = buffer.readUInt16LE(0);
    if (packetLength < 20 || buffer.length < 20) {
        throw new Error(`Invalid Z21 system state response length: packetLength=${packetLength}, bufferLength=${buffer.length}`);
    }
    const dataOffset = 4;
