...
export default function LayoutPage({ onGoHome }) {
    const [locoDialogOpened, setLocoDialogOpened] = useState(false);
    const [locos, setLocos] = useState([]);
    const [tool, setTool] = useState({ mode: "cursor", elementType: "general" });
    ...
    const [commandCenter, setCommandCenter] = useState(new CommandCenter());
    ...
    const [locoPanelCollapsed, setLocoPanelCollapsed] = useState(() => {
        try {
    ...
    const [propertyPanelCollapsed, setPropertyPanelCollapsed] = useState(() => {
        try {
    ...
    const saveCommandCentersToServer = async (items) => {
        try {
    ...
    const createLayoutSnapshot = useCallback((source) => {
    ...
    const selectedElementChanged = (e) => {
        setSelectedElement(e);
    };
    const handleUpdateSelectedElement = (_updated) => {
        setInavalidateCounter((prev) => prev + 1);
    };
    ...
    const handleCommandCenterSaved = (cc) => {
        //loadCommandCentersFromServer();
        setCommandCenter(cc);
    };
    return (<>
      <CommandCenterDialog opened={commandCenterOpened} onClose={() => setCommandCenterOpened(false)} onSave={handleCommandCenterSaved} commandCenter={commandCenter}/>

      <LocoDialog opened={locoDialogOpened} onClose={() => setLocoDialogOpened(false)} onSaved={async () => {
            await refreshLocos();
        }}/>

      <ElementPickerDialog opened={pickerOpened} onClose={() => setPickerOpened(false)} onPick={(elementType) => {
            setTool({ mode: "draw", elementType });
        }}/>

      <SettingDialog opened={settingsDialogOpened} onClose={() => setSettingsDialogOpened(false)}/>

      <AppShell header={{ height: 50 }} footer={{ height: 40 }} padding="xs">
        <AppShell.Header>
          <TopMenuBar editMode={editMode} onEditModeChange={setEditMode} onGoHome={onGoHome} onOpenLocos={() => setLocoDialogOpened(true)} locoPanelCollapsed={locoPanelCollapsed} onToggleLocoPanel={() => setLocoPanelCollapsed((v) => !v)} propertyPanelCollapsed={propertyPanelCollapsed} onTogglePropertyPanel={() => setPropertyPanelCollapsed((v) => !v)} tool={tool} onCursorToolClick={() => setTool({ mode: "cursor", elementType: tool.elementType })} onOpenElementPicker={() => setPickerOpened(true)} onSaveLayout={saveLayoutToServer} onLoadLayout={loadLayoutFromServer} canUndo={canUndo} canRedo={canRedo} onUndo={undo} onRedo={redo} onSettingsClick={handleSettingClick} onDeleteToolClick={() => setTool({ mode: "delete", elementType: tool.elementType })} onFitLayout={onFitLayout} onOpenCommandCenterDialog={() => setCommandCenterOpened(true)}/>
        </AppShell.Header>

        <AppShell.Main>
          <Stack gap="xs" h="calc(100vh - 60px - 34px - 20px)">
            <Group gap="xs" wrap="nowrap" align="stretch" style={{ flex: 1, minHeight: 0 }}>
              <Box style={{
            ...
            position: "relative",
                ...
                <PanelHandle side="left" collapsed={locoPanelCollapsed} onToggle={() => setLocoPanelCollapsed((v) => !v)} style={{ left: 1 }}/>

                <PanelHandle side="right" collapsed={propertyPanelCollapsed} onToggle={() => setPropertyPanelCollapsed((v) => !v)} style={{ right: 1 }}/>
            ...
            border: "1px solid var(--mantine-color-dark-4)",
                    ...
                    <TrackCanvas editMode={editMode} tool={tool} layout={layout} onLayoutChange={setLayout} onBeforeLayoutChange={pushHistorySnapshot} selectedElement={selectedElement} onSelectedElementChange={selectedElementChanged} invaildateCounter={invaildateCounter} fitCounter={fitCounter}/>
                  </Box>
                </Card>
              </Box>
            ...
            flexShrink: 0,
                ...
                {!propertyPanelCollapsed && (<Card withBorder radius="xs" p="xs" h="100%">
                    <RightPropertyPanel selectedElement={selectedElement} invalidate={invaildateCounter} onUpdateSelectedElement={handleUpdateSelectedElement} editMode={editMode}/>
                  </Card>)}
              </Box>
            </Group>
          </Stack>
