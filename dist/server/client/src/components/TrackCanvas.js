    ...
    try {
        ...
        return {
            scale: typeof parsed.scale === "number" && Number.isFinite(parsed.scale)
                ...
                : 1,
            offsetX: typeof parsed.offsetX === "number" && Number.isFinite(parsed.offsetX)
                ? parsed.offsetX
                : 0,
            offsetY: typeof parsed.offsetY === "number" && Number.isFinite(parsed.offsetY)
                ? parsed.offsetY
                : 0,
        };
    }
...
export default function TrackCanvas({ editMode = false, tool, layout, onLayoutChange, onBeforeLayoutChange, selectedElement, onSelectedElementChange, invaildateCounter, fitCounter }) {
    ...
    const viewRef = useRef(loadSavedViewState());
    const panRef = useRef({
        isPanning: false,
        lastX: 0,
        lastY: 0,
    ...
    useEffect(() => {
        ...
        ctx.scale(dpr, dpr);
        drawScene(ctx, canvasSize.width, canvasSize.height, editMode, colorScheme, viewRef.current, mouseGrid, tool, hoverGrid, currentCursor, layout, settings, dragRef.current.elementId ?? undefined, selectedElement ?? undefined, getSelectionRect(selectionRef.current));
    ...
    useEffect(() => {
        ...
        const handleMouseDown = (ev) => {
            ...
            const mouseX = ev.clientX - rect.left;
            const mouseY = ev.clientY - rect.top;
            const grid = screenToGrid(mouseX, mouseY, viewRef.current, currentLayout.gridSize);
            ...
            if (hitElement) {
                if (ev.ctrlKey) {
                    ...
                    const allSelected = getAllLayoutElements(currentLayout).filter((el) => el.selected);
        ...
        const handleMouseMove = (ev) => {
            ...
            const mouseY = ev.clientY - rect.top;
            const grid = screenToGrid(mouseX, mouseY, viewRef.current, currentLayout.gridSize);
            setMouseGrid((prev) => prev.x === grid.x && prev.y === grid.y ? prev : { x: grid.x, y: grid.y });
            ...
            if (dragRef.current.isDraggingElement && dragRef.current.elementId) {
                ...
                const dy = grid.y - dragRef.current.startMouseGridY;
                const all = getAllLayoutElements(currentLayout);
                const selectedIds = new Set(dragRef.current.draggedElements.map((item) => item.id));
                for (const item of dragRef.current.draggedElements) {
                    const el = all.find((e) => e.id === item.id);
                    if (!el)
                        continue;
                    ...
                    const occupied = currentLayout.getLayeredElement(el, nextX, nextY);
                    if (occupied && !selectedIds.has(occupied.id)) {
                        setHoverGrid((prev) => prev?.x === nextX && prev?.y === nextY
                            ? prev
                            : { x: nextX, y: nextY });
        ...
        const stopInteraction = () => {
            ...
            if (selectionRef.current.isSelecting) {
                ...
                if (rect) {
                    const selected = applySelectionRect(layoutRef.current, rect, selectionRef.current.additive);
        ...
        const handleMouseUp = (ev) => {
            if (!editModeRef.current && ev.button == 0) {
                ...
                const mouseX = ev.clientX - rect.left;
                const mouseY = ev.clientY - rect.top;
                const grid = screenToGrid(mouseX, mouseY, viewRef.current, layoutRef.current.gridSize);
        ...
        const handlePointerMove = (ev) => {
            ...
            if (!pointerPanRef.current.isTouchPanning)
                return;
            if (pointerPanRef.current.activePointerId !== ev.pointerId)
                return;
            ev.preventDefault();
            const dx = ev.clientX - panRef.current.lastX;
            const dy = ev.clientY - panRef.current.lastY;
    ...
    useEffect(() => {
        const handleKeyDown = (ev) => {
            ...
            const selectedElements = getAllLayoutElements(currentLayout).filter((el) => el.selected);
...
function createCursorElement(tool) {
...
function drawScene(ctx, width, height, editMode, colorScheme, view, mouseGrid, tool, hoverGrid, currentCursor, layout, settings, dragId, selected, selectionRect) {
...
function drawBackground(ctx, width, height, isDark) {
    ctx.fillStyle = isDark ? "#313131" : "#F8F9FA";
    ctx.fillRect(0, 0, width, height);
}
function drawGrid(ctx, width, height, gridSize, isDark, view) {
...
function drawInfo(ctx, width, editMode, isDark, scale, mouseGrid, tool, currentCursor) {
    const px = 20;
    const toolText = tool.mode === "cursor" ? "Kurzor" : `Rajz: ${tool.elementType}`;
    const rotationText = tool.mode === "draw" && currentCursor
...
function screenToGrid(screenX, screenY, view, gridSize) {
...
function drawSelectionRect(ctx, gridSize, rect, scale) {
...
function getAllLayoutElements(layout) {
    return [
...
function applySelectionRect(layout, rect, additive = false) {
    const n = normalizeSelectionRect(rect);
...
function fitLayoutToView(layout, view, canvasWidth, canvasHeight) {
    ...
    let newScale = Math.min(availableWidth / worldWidth, availableHeight / worldHeight);
