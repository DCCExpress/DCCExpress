...
import { ApiCommands, CommandCenterTypes, rbus, Z21Directions, SpeedModes, } from "../../common/src/dcc";
import { CommandCenter } from "./CommandCenter.js";
// import dgram from "dgram";
import { log, arrayToHex, logError } from '../utility.js';
...
export class Z21CommandCenter extends CommandCenter {
    ...
    constructor(name, ip, port, init) {
        ...
        this.ip = ip;
        this.port = port;
        this.udpClient = new UDPClient(this.name, this.ip, this.port, (buffer, rinfo) => {
            ...
            if (i != rinfo.size) {
                ...
                logError("payload:", i);
                broadcast({ type: ApiCommands.alert, data: "Z21 on message: eltérő méretek!" });
            }
        });
    }
    getConnectionString() {
    ...
    parse(data) {
        ...
        if (header == 0x40) {
            ...
            if (len == 0x09 && xheader == cLAN_X_TURNOUT_INFO0x43) {
                ...
                broadcast({ type: ApiCommands.turnoutInfo, data: turnoutInfo });
            ...
            //========================================
                ...
                broadcast({ type: ApiCommands.locoInfo, data: loco });
        ...
        //========================================
            ...
            if (header == 0x80) {
                ...
                broadcast({ type: ApiCommands.rbusInfo, data: rbus });
            ...
            this.powerInfo.programmingModeActive = (info & 0x20) == 0x20;
            //const pi: iPowerInfo = { info: data.readUInt8(5), cc: commandCenters.getDevice(this.uuid) }
            broadcast({ type: ApiCommands.systemInfo, data: sysdata });
            broadcast({ type: ApiCommands.powerInfo, data: this.powerInfo });
        }
